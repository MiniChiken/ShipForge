...
$result=Join-Path $here "loc_result.json"
Remove-Item -LiteralPath $result -ErrorAction SilentlyContinue
Push-Location $clientTq
try { & (Join-Path $clientTq "bin64\exefile.exe") /py (Join-Path $here "loc_worker.py") $result (Join-Path $here "loc_request.json") /inherit } finally { Pop-Location }
