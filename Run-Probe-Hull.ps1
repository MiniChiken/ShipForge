...
$result=Join-Path $here "probe_hull_result.json"
Remove-Item -LiteralPath $result -ErrorAction SilentlyContinue
Push-Location $clientTq
try { & (Join-Path $clientTq "bin64\exefile.exe") /py (Join-Path $here "probe_hull.py") $result (Join-Path $here "probe_request.json") /inherit } finally { Pop-Location }
